...
PROC IMPORT datafile="O:\Programs\Other Pathogens or Responses\2019-nCoV\Special Projects Surveillance\Vanadata Data Sets\Pop_Denom.csv"
...
DATA HHS_Hosp_clean_1;
	...
	/*drop deaths_covid hospital_onset_covid Total_previous_cases_admit;*/ /*keeping in case others need these, per discussion with Kaitlin*/
RUN; 
...
PROC SQL;
   CREATE TABLE NYC_data2 AS 
   SELECT date_of_interest as date,
		  case_count,
		  hospitalized_count as total_cases_hosp,
		  death_count, 
      	  case_count_7day_avg,
		  hosp_count_7day_avg,
...
DATA NYData;
	set NYC_data2;
	format location $25. location_type $25. source $15. section $25. omi_wave_dt yymmdd10.;
	location = 'New York City';
	location_type = 'City';
	source = 'NYC Health';
	section = 'Omicron';
	Omi_wave_dt = '15Mar2022'd;
	Omi_daydiff = Date - Omi_wave_dt;
RUN;


PROC SQL;
create table NYC_Hosp_Pop
as select v.*, e.*

	from work.NYdata v
	left join HHS_Populations e on v.location = e.city
...
DATA NYC_Clean; 
	...
	daily_hosp_per100k =(total_cases_hosp/pop_denom)*100000;
	cases_per100k = (case_count/pop_denom)*100000;
	cases_per_100k_7day = (case_count_7day_avg/pop_denom)*100000;

	drop state_name state city;
RUN;

PROC SORT data=NYC_Clean; by date; RUN; 


/****** LONDON DATA ******/

/***Pull data for London Hospitalizations***/
filename LondonHP temp;
proc http
url="https://api.coronavirus.data.gov.uk/v2/data?areaType=nhsRegion&areaCode=E40000003&metric=cumAdmissions&metric=newAdmissions&metric=newAdmissionsChange&metric=hospitalCases&format=csv"
method= "GET"
out=LondonHP;
run;
proc import file=LondonHP
out=Londonhospdata1
dbms=csv
replace;
run;
...
data LondonHospData; 
	set Londonhospdata1;
	format source $15. location $25. location_type $25. omi_wave_dt yymmdd10.; 
	Source = 'UK NHS';
	Location_type = 'city';
	Section = 'Omicron';
	Omi_Wave_dt = '01Mar2022'd; 
	Omi_daydiff = Date - Omi_wave_dt;
	rename areaName = Location hospitalCases=total_cases_hosp;
	if date >= '1Dec2021'd then output;
	drop areacode areaType; 
run; 

PROC SORT data=LondonHospData; by date; RUN;

/***Pull data for London Cases***/
filename LondonC temp;
proc http
url="https://api.coronavirus.data.gov.uk/v2/data?areaType=region&areaCode=E12000007&metric=cumCasesByPublishDate&metric=cumCasesBySpecimenDate&metric=newCasesBySpecimenDate&metric=newCasesByPublishDate&format=csv"
method= "GET"
out=LondonC;
run;
proc import file=LondonC
out=Londoncasedata1
dbms=csv
replace;
run;

data LondonCaseData2;
	set Londoncasedata1;
	format source $15. location $25. location_type $25. omi_wave_dt yymmdd10.; 
	Source = 'UK NHS';
	Location_type = 'city';
	...
	drop areaCode areaType; 
run; 
PROC SORT data=LondonCaseData2; by date; RUN;

/* Combo Cases & Hosp into one datafile */;

PROC SQL;
create table London_case_hosp
as select distinct v.*, e.*

	from work.LondonHospData v 
	left join work.LondonCaseData2 e on v.date= e.date
...
PROC SQL;
create table London_case_hosp2
as select v.*, e.*

	from work.London_case_hosp v 
	left join HHS_Populations e on v.location = e.city
...
DATA London_case_hosp_clean;   /* IN FINAL EXPORT FILE */
	set London_case_hosp2;
	format section $30. category $30. omi_wave_dt yymmdd10.;

	section = 'Cases & Hospitalization';
	category = 'Omicron';
	Omi_Wave_dt = '01Mar2022'd; 
	Omi_daydiff = Date - Omi_wave_dt;
	cases_per100k = (newCasesBySpecimenDate/pop_denom)*100000;
	daily_hosp_per100k = (total_cases_hosp/pop_denom)*100000;
	daily_hospadmit_per100k = (newAdmissions/pop_denom)*100000;

	drop state_name state city cumCasesByPublishDate cumCasesBySpecimenDate newCasesByPublishDate;
RUN;

/****** UK DATA ******/

/***Pull data for UK Currently Hospitalized (hospitalCases)***/
filename UK_Hosp temp;
proc http
url="https://api.coronavirus.data.gov.uk/v2/data?areaType=overview&metric=hospitalCases&format=csv"
out=UK_Hosp;
RUN; 

proc import file=UK_Hosp
out=UK_Hosp_data
dbms=csv
replace;
run;
...
DATA UK_Current_hosp;
	set WORK.UK_Hosp_data; /*Patients In Hospital - All UK */;
	/*date, hospitalCases */;

	format source $15. location $30. location_type $25. omi_wave_dt yymmdd10.; 
	Source = 'UK NHS';
	Location = 'UK';
	Location_type = 'Country';
	pop_denom = 67508936; /*Pop_denom pulled from OWID_Clean */;

	daily_hosp_per100k = (hospitalCases/pop_denom)*100000;
	Omi_Wave_dt = '01Mar2022'd; 
	Omi_daydiff = Date - Omi_wave_dt;
	
	if date >= '1DEC2021'd then output;
	drop areaType areaCode areaName;
RUN;

/***Pull data for UK Regions Currently Hospitalized (hospitalCases)***/

filename UK_Reg temp;
proc http
url="https://api.coronavirus.data.gov.uk/v2/data?areaType=nation&metric=hospitalCases&format=csv"
out=UK_Reg;
RUN; 

proc import file=UK_Reg
out=UK_Reg_Hosp_data
dbms=csv
replace;
run;

DATA UK_Regions_Hosp; 
	set WORK.UK_Reg_Hosp_data; /*Patients In Hosp - UK Regions */;
	/*date, hospitalCases */;
	format source $15. location $30. location_type $25. omi_wave_dt yymmdd10.; 
	Source = 'UK NHS';
	Location_type = 'Country';
	Omi_Wave_dt = '01Mar2022'd; 
	Omi_daydiff = Date - Omi_wave_dt;
	if areaName= 'Norther' then location = 'Northern Ireland'; 
		else if areaName = 'Scotlan' then location = 'Scotland';
		else if areaName = 'England' then location = 'England';
		else if areaName = 'Wales' then location = 'Wales';
	if date >= '1DEC2021'd then output;
	drop areaType areaCode areaName;
RUN;

DATA UK_Regions_Hosp_1;
	set UK_Regions_Hosp;

	/*population from 2021: https://populationdata.org.uk/ */;
	 if location = 'Northern Ireland' then pop_denom = 1895510;
	 	else if location = 'Scotland' then pop_denom = 5466000;
		else if location = 'England'  then pop_denom = 56550138;
		else if location = 'Wales'    then pop_denom = 3169586;

	daily_hosp_per100k = (hospitalCases/pop_denom)*100000;
RUN; 

DATA UK_Currently_HOSP; 	/*REGIONS & ALL UK - date, hospitalCases */;
	set UK_Current_hosp
		UK_Regions_Hosp_1; 
RUN;
...
/***Pull data for UK CASES & Hosptial Admission, UK REGIONS ***/
filename UK temp;
proc http
url="https://api.coronavirus.data.gov.uk/v2/data?areaType=nation&metric=cumAdmissions&metric=newAdmissions&metric=newCasesByPublishDate&metric=newCasesBySpecimenDate&metric=cumCasesByPublishDate&format=csv"
out=UK;
run;
...
data UKData_1; 
	set UK_data; /*newAdmissions newCasesbySpecimenDate*/;
 	format source $15. location $30. location_type $25.; 
	Source = 'UK NHS';
	Location_type = 'Country';
	...
	drop areaCode areaName areaType; 
	if date >= '1Dec2021'd then output;
run;

data UKData;  /* UK REGIONS */;
	set UKData_1;
	format omi_wave_dt yymmdd10.;
	daily_hospadmit_per100k = (newAdmissions/pop_denom)*100000;
	cases_per100k = (newCasesBySpecimenDate/pop_denom)*100000; 
	...
	drop cumAdmissions cumCasesByPublishDate newCasesByPublishDate; 
RUN;


DATA UK_Data2; /*ALL UK*/;
	set WORK.'data_2022_Dec_22 (7)'n; /*HOSP ADMISSION - ALL UK */;
	...
	Location_type = 'Country';
	if date >= '1DEC2021'd then output;
	drop areaType areaCode areaName cumAdmissions;
RUN;
...
DATA UK_Admissions_Cases; /*REGION & UK - hospAdmit & Cases */;
	set UKData
		UKData_2;
RUN;
...
DATA UK_DATA_CASES; 
	set WORK.'data_2022_Dec_22 (9)'n; /*ALL UK CASES DAILY*/;
	...
	if date >= '1DEC2021'd then output;
	drop areaType areaCode areaName cumCasesBySpecimenDate 
		 cumFirstEpisodesBySpecimenDate newReinfectionsBySpecimenDate
		 newFirstEpisodesBySpecimenDate cumReinfectionsBySpecimenDate
;
RUN;

PROC SQL;
create table UK_ADMIT_CASES
as select distinct v.*, e.*

	from work.UK_DATA_CASES v 
	left join work.UK_Admissions_cases e on v.date = e.date and v.location = e.location
...
/* Combo UK Admissions, Cases & Hosp into one datafile */;

PROC SQL;
create table UK_Case_Admit
as select distinct v.*, e.*

	from work.UK_Currently_HOSP v 
	left join work.UK_Cases_Admit e on v.date=e.date and v.location=e.location
...
PROC SQL;
create table UK_Case_Hosp_Admit_Clean /*IN FINAL OUTPUT */
as select distinct v.*, e.*

	from work.UK_Case_Admit v 
	left join work.UK_CURRENTLY_HOSP e on v.date=e.date and v.location=e.location
...
Data dashboard_1;
	...
	set work.OWID_CLEAN /*OWID data */
		...
		work.NYC_CLEAN /* all NYC Data */
		work.London_case_hosp_clean /*all London DATA */
		work.UK_CASE_HOSP_ADMIT_CLEAN /*all UK DATA */
